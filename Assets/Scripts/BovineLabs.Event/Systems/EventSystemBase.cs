...
namespace BovineLabs.Event.Systems
{
    ...
    public abstract partial class EventSystemBase : SystemBase
    {
        ...
        private readonly List<EventContainer> _containers = new List<EventContainer>();
        ...
        {
            var handles = new NativeArray<JobHandle>(this._containers.Count, Allocator.TempJob);

            for (var i = 0; i < this._containers.Count; i++)
            {
                ...
                handles[i] = this._streamBus.ReleaseStreams(this, container.ExternalReaders, handles[i]);
                handles[i] = this._streamBus.ReleaseStreams(this, container.DeferredExternalReaders, handles[i]);
            ...
            this._streamBus.Unsubscribe(this);
        ...
        protected override void OnUpdate()
        {
            ...
            for (var i = 0; i < this._containers.Count; i++)
            {
                ...
                handles[i] = this._streamBus.AddStreams(this, container.Type, container.Streams, handles[i]);
                Profiler.EndSample();
        ...
        {
            if (!this._types.TryGetValue(type, out var index))
            {
                index = this._types[type] = this._containers.Count;
                this._containers.Add(new EventContainer(type, this.UsePersistentAllocator));
            }
